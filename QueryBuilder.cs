    ...
    public class QueryBuilder : IDisposable
    {
        ...
        public void Create<T>(T obj) where T : IClassModel
        {
          ...
          #region String Chaos
            ...
            string insertStatement = $"INSERT INTO {typeof(T).Name} (";

            for (int i = 0; i < names.Count; i++)
            {
                if (i == names.Count - 1)
                {
                    insertStatement += $"{names[i]}) VALUES (";
                }
                else
                {
                    insertStatement += $"{names[i]}, ";
                }
            }
            ...
            /*
                string column = string.Join(", ", names);
                string parameters = string.Join(", ", values);
                var command = connection.CreateCommand();
                command.CommandText = $"INSERT INTO {typeof(T).Name} ({column}) VALUES ({parameters})";
                var reader = command.ExecuteNonQuery();
            */
